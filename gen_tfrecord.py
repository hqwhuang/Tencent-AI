...
def run(index, args):
    with open("/cos_person/training_data_v2/train_serialize_{}.csv".format(index), "r") as f:
        lines = f.readlines()
        ...
        writer = tf.python_io.TFRecordWriter("/cos_person/training_data_tfrecord_v2/train_tfrecord_{}.gz".format(index), options=options)
        for line in values:

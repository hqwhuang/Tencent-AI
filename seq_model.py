...
feature_set_map = {
    "sequencev1": feature_set_v3,
    "sequencev2": feature_set_v3,
...
def main(argv):
    ...
    def model_fn(features, labels, mode, params):
        ...
        with tf.variable_scope("deep"):
            ...
            logits = tf.cond(tf.math.greater(tf.train.get_global_step(), tf.constant(188000, dtype=tf.int64)),
                lambda: tf.nn.bias_add(fm_logits + dense_logits, global_bias),
                lambda: tf.nn.bias_add(fm_logits, global_bias)
            )
            logits = tf.nn.bias_add(fm_logits + dense_logits, global_bias)
